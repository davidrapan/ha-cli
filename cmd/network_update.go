...
func init() {
	...
	networkUpdateCmd.Flags().String("ipv6-addr_gen_mode", "", "IPv6 address generation mode: eui64|stable-privacy")
	...
	networkUpdateCmd.RegisterFlagCompletionFunc("ipv6-addr_gen_mode", ipAddrGenModeCompletions)
...
func helperIpConfig(version string, cmd *cobra.Command, options map[string]any) {
	args := []NetworkArg{
		...
		{Arg: version + "-addr_gen_mode", ApiKey: "addr_gen_mode"},
